...
class SingleQuery(MongoDBModel):
    ...
    def get_by_id(cls, _id: int) -> SingleQuery:
        return cls.from_json(cls._collection.find_one({"_id": _id}, cls.fields))
    ...
    def datetime(self, _datetime: datetime):
        self.__datetime = _datetime
        self.__class__._collection.update_one({'_id': self._id}, {'$set': {'datetime': self.datetime}})
